...
char* load_chord_class()
{
	...
	// set the global reference to prevent deletion from GC
	chordClass = (*env)->NewGlobalRef(env, localChordClass);

	// Defining all the methods so we can have access to them in GO.
...
// wrapper for second constructor
...
char** call_method_get_all_keys(jobject chordObject, char** out_error)
{
	...
	for(i = 0; i < len; i++){
		...
		jstring j_str = (*env)->GetObjectArrayElement(env, jresult, i);
		...
		if(new_str == NULL){
			...
			char* err_msg = malloc(sizeof(char) * 14);
			strcpy(err_msg, "strdup failed");
			*out_error = err_msg;
			did_fail = 1;
			goto cleanup_loop;
		}
		...
		cleanup_loop:
			...
			// Free the const string - release
			if(c_str_defined) (*env)->ReleaseStringUTFChars(env, j_str, c_str);

			// Free the jstring - decref
...
void call_method_delete(jobject chordObject, char* key, char** out_error){
	...
	// Call the get method
	(*env)->CallObjectMethod(env, chordObject, methodDelete, jkey);
	if ((*env)->ExceptionCheck(env)) {
		*out_error = get_exception_message(env);
		goto cleanup;
	}
